...
export default function AdminLayout({ children, title, action }) {
  ...
  return (
    <div className="min-h-screen bg-slate-50">
      ...
      <div className="md:hidden bg-white border-b border-slate-200 px-6 py-2 flex justify-center gap-2 sticky top-[72px] z-20">
        <Link
          to="/admin"
          className={`px-3 py-1.5 rounded-md text-xs font-semibold transition-colors flex items-center ${isActive("/admin") && !isActive("/admin/customers") ? "bg-slate-900 text-white" : "text-slate-700 hover:bg-slate-100"}`}
          data-testid="mobile-nav-complaints"
        >
          <ClipboardList className="h-3.5 w-3.5 mr-1" /> Complaints
        </Link>
        <Link
          to="/admin/customers"
          className={`px-3 py-1.5 rounded-md text-xs font-semibold transition-colors flex items-center ${isActive("/admin/customers") ? "bg-slate-900 text-white" : "text-slate-700 hover:bg-slate-100"}`}
